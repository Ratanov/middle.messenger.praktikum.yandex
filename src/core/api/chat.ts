...
export default class ChatApi {
  ...
  ): Promise<IChat.GETChatsResponse[] | ApiError> {
    return transport.get('/chats', { data });
  }

  async createChat(
    data?: IChat.CreateChatRequest,
  ): Promise<IChat.CreateChatResponse | ApiError> {
  ...
  async addUserToChat(
    data: IChat.AddUserToChatRequest,
  ...
  async getChatUsers(
    data: IChat.GetChatUsersRequest,
  ): Promise<IChat.GetChatUsersResponse[] | ApiError> {
    return transport.get<IChat.GetChatUsersResponse[]>(
      `/chats/${data.id}/users`,
    );
  }
  ...
  ): Promise<IChat.GetChatTokenResponse | ApiError> {
    return transport.post<IChat.GetChatTokenResponse>(
      `/chats/token/${data.chatId}`,
      { data },
    );
  }
}
